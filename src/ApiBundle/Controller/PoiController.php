...
class PoiController extends Controller
{
    ...
    public function getPoiAction($id)
    {
        ...
        if($poi) {
            ...
            foreach ($infos['aggregations']['age']['buckets'] as $bucket)
                if(!empty($bucket['key']))
                    $age[$bucket['key']] = $bucket['score']['value'];

            return new JsonResponse(['code' => 200, 'infos' => $ret, 'accessible' => $poi->getHandicap(), 'age' => $age]);
        }
        return new JsonResponse(['code' => 404, 'message' => 'Poi not found']);
    }

    /**
     * @Route("/genPois")
     */
    public function genPoisAction()
    {
        if("147.210.245.180" != $_SERVER['REMOTE_ADDR'])
            return new JsonResponse(['code' => 418, 'message' => 'I\'m a teapot']);
        ...
        foreach (range(1,500) as $key) {
            $poi = new Poi();
            $poi->setCaption('Generated poi #'.$key);
            $poi->setAddress('Address of Generated poi #'.$key);
            $poi->setLat(rand(4475, 4490)/100);
            $poi->setLon(rand(-070, -047)/100);
            $poi->setType('default');
            $em->persist($poi);
        }
    ...
    /**
     * @Route("/clonePoisToElastic")
     * @ApiDoc(
     *   description="Clones all PoIs into the elasticsearch database using a Bulk request",
     *   statusCodes={
     *     200="Data inserted successfully"
     *   }
     * )
     */
    public function clonePoisToElastic()
    {
        if(!in_array($_SERVER['REMOTE_ADDR'], ["147.210.245.180", "46.193.16.47", "78.119.159.97"]))
            return new JsonResponse(['code' => 418, 'message' => 'I\'m a teapot']);
        ...
        foreach ($pois as $poi) {
            ...
            $params['body'][] = [
                ...
                'city' => $poi->getCityLat().";".$poi->getCityLon(),
                'caption' => $poi->getCaption(),
                'address' => $poi->getAddress(),
                'type' => $poi->getType(),
                'accessible' => $poi->getHandicap(),
                'beacons' => $beacons
            ];
        }
        SessionManager::_getClient()->bulk($params);
     ...
     * @Route("/assignCity")
     */
    public function assignCityAction()
    {
        if("147.210.245.180" != $_SERVER['REMOTE_ADDR'])
            return new JsonResponse(['code' => 418, 'message' => 'I\'m a teapot']);
